class SegTree:
    ...
    def query(self, node, start, end, index):
        if start == end:
            return start
        mid = (start + end) // 2
        if index <= self.seg_tree[2 * node]:
            return self.query(node * 2, start, mid, index)
        else:
            return self.query(node * 2 + 1, mid + 1, end, index - self.seg_tree[2 * node])
...
for i in range(n):
    ...
    num = st.query(1, 1, n, index)
    st.update(num, 1, 1, 1, n)
    res.append(num)
print('<', end='')
print(*res, sep=', ', end='')
print('>')